...
def resize_norm_img_chinese(img, image_shape):
    imgC, imgH, imgW = image_shape # 3,32,320
    # todo: change to 0 and modified image shape
    max_wh_ratio = imgW * 1.0 / imgH  # 10
    h, w = img.shape[0], img.shape[1]   # 100,150
    ratio = w * 1.0 / h  # 1.5
    max_wh_ratio = max(max_wh_ratio, ratio)  # 10
    imgW = int(32 * max_wh_ratio)    # 320
    if math.ceil(imgH * ratio) > imgW:
        resized_w = imgW
    else:
        resized_w = int(math.ceil(imgH * ratio))
    ...
    resized_image /= 0.5
    padding_im = np.zeros((imgC, imgH, imgW), dtype=np.float32)
    #padding_im[:, :, int((imgW-resized_w)/2):int((imgW+resized_w)/2)] = resized_image
    padding_im[:, :, 0:resized_w] = resized_image  #原代码
    return padding_im
...
def warp(img, ang, use_tia=True, prob=0.7):
    """
    warp
    """
    h, w, _ = img.shape
