...
def train(config,
    ...
    for epoch in range(start_epoch, epoch_num + 1):
        ...
        train_dataloader3 = build_dataloader(
            config, 'Train', device, logger, seed=epoch)
        train_batch_cost = 0.0
        train_reader_cost = 0.0
        batch_sum = 0
        batch_start = time.time()
        for idx, batch in enumerate(train_dataloader3):
            s1= time.time()
            '''for i in range(len(temp)):
                if len(temp[i])>5:
                    flag=1
        ...
        for idx, batch in enumerate(train_dataloader2):
            ...
            if dist.get_rank() == 0 and (
                (global_step > 0 and global_step % print_batch_step == 0) or
                (idx >= len(train_dataloader2) - 1)):
                logs = train_stats.log()
                strs = '2 epoch: [{}/{}], iter: {}, {}, reader_cost: {:.5f} s, batch_cost: {:.5f} s, samples: {}, ips: {:.5f}'.format(
                    epoch, epoch_num, global_step, logs, train_reader_cost /
                    print_batch_step, train_batch_cost / print_batch_step,
                    batch_sum, batch_sum / train_batch_cost)
                logger.info(strs)
