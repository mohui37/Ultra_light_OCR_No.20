...
def train(config,
    ...
    for epoch in range(start_epoch, epoch_num + 1):
        num+=1
        train_dataloader2 = build_dataloader(
            config, 'Train2', device, logger, seed=epoch)
        ...
        for idx, (batch,batch2) in enumerate(zip(train_dataloader,train_dataloader2)):
            s1= time.time()
            ...
            #随机拼接-数据增强
            img_batch=batch2[0].numpy()
            label_batch=batch2[1].numpy()
            label_lengths=batch2[2].numpy()
            ...
            if cal_metric_during_train:  # only rec and cls need
                batch2 = [item.numpy() for item in batch2]
                post_result = post_process_class(preds, batch2[1])
                eval_class(post_result, batch2)
                metric = eval_class.get_metric()
                train_stats.update(metric)
            ...
            if dist.get_rank() == 0 and (
                ...
                (idx >= len(train_dataloader) - 1)):
                logs = train_stats.log()
                strs = 'epoch_2: [{}/{}], iter: {}, {}, reader_cost: {:.5f} s, batch_cost: {:.5f} s, samples: {}, ips: {:.5f}'.format(
                    epoch, epoch_num, global_step, logs, train_reader_cost /
                    print_batch_step, train_batch_cost / print_batch_step,
                    batch_sum, batch_sum / train_batch_cost)
                logger.info(strs)
